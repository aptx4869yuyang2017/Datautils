...
from typing import Dict, Any, Union, List  # 添加 List 导入
...
class FileStorage:
    ...
    def save_to_json(self, data: Dict[str, Any], filename: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(f'开始保存 {file_path}')
    ...
    def sql_from_parquet(self, table_paths: Dict[str, Union[str, List[str]]], sql: str) -> pd.DataFrame:
        """
        从多个Parquet文件执行SQL查询，支持多表联查
        :param table_paths: 表名和文件路径的字典，格式为 {'table_name': 'path' or ['path1', 'path2']}
        :param sql: SQL查询语句
        ...
        try:
            ...
            for table_name, paths in table_paths.items():
                ...
                # 创建视图
                conn.execute(
                    f"CREATE VIEW {table_name} AS SELECT * FROM parquet_scan([{', '.join(f''''{path}' ''' for path in path_list)}])"
                )

            # 执行SQL查询
            result = conn.execute(sql).fetchdf()

            self.logger.info('SQL查询执行成功')
            return result
